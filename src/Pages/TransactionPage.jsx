...
const AddTransactionModal = ({ onClose, onSuccess }) => {
  ...
  const [form, setForm] = useState({
    ...
    note: "",
    date: "",
    isRecurring: false,      // hiện chưa dùng (FE chưa gửi recurringDay)
    receiptImages: [],       // thunk cần mảng -> OK
  });
  ...
  const handleChange = (e) => {
    ...
    if (type === "checkbox") {
      ...
      setForm({ ...form, receiptImages: Array.from(files) }); // luôn là mảng
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(createTransaction(form)).unwrap(); // gửi object đúng cấu trúc thunk cần
      onSuccess();   // reload lại danh sách
      onClose();     // đóng modal
    } catch (err) {
      alert("Không thể thêm giao dịch: " + err);
      console.error(err);
    }
  };
