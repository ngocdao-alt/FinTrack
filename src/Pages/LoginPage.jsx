    ...
    <>
      ...
      <div className="lg:hidden min-h-screen flex flex-col bg-gradient-to-b from-white via-indigo-200 to-indigo-600">
        ...
        <div className="flex flex-col items-center justify-center grow px-4">
          <img src={phoneImg} alt="Phone" className="w-60 mt-4 mb-4 select-none pointer-events-none" />
          <form onSubmit={handleSubmit} className="w-full max-w-sm flex flex-col gap-4">
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email"
              className="w-full rounded-lg px-4 py-3 bg-white outline-none ring-1 ring-gray-200 focus:ring-2 focus:ring-indigo-400 placeholder-gray-500" required />
            {isRegister && (
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password"
                className="w-full rounded-lg px-4 py-3 bg-white outline-none ring-1 ring-gray-200 focus:ring-2 focus:ring-indigo-400 placeholder-gray-500 pr-12" required />
      ...
      <div className="hidden lg:flex min-h-screen w-full relative">
        ...
        <div className="w-1/2 flex flex-col items-center justify-center px-20 bg-white lg:pl-15 lg:pr-0">
          <form onSubmit={handleSubmit} className="w-full max-w-lg ml-auto flex flex-col gap-6 pt-12">
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email"
              className="w-full rounded-lg px-4 py-3 bg-white outline-none placeholder-gray-500 text-lg border-2 border-black-1000" required />
            {isRegister && (
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Name"
                className="w-full rounded-lg px-4 py-3 bg-white outline-none placeholder-gray-500 text-lg border-2 border-black-600" required />
            )}
            <div className="relative">
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password"
                className="w-full rounded-lg px-4 py-3 bg-white outline-none placeholder-gray-500 text-lg border-2 border-black-600" required />
            ...
            <div className="flex justify-end">
              <button type="submit" className="w-fit px-6 py-2 rounded-xl bg-indigo-200 hover:bg-indigo-300 active:bg-indigo-400 text-gray-800 font-medium transition text-base">
        ...
        <div className="relative mt-[10%] mr-[15%] ml-[20%] rounded-4xl border-gray-300 bg-gradient-to-b from-indigo-600 via-indigo-200 to-white flex items-center justify-center lg:w-1/2">
          <img src={phoneImg} alt="Phone" className="w-[320px] lg:w-[420px] drop-shadow-2xl md:absolute md:-left-50 md:z-100" />
