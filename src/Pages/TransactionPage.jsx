...
const TransactionPage = () => {
  ...
  const { transactions, loading, total, totalPage, page } = useSelector((s) => s.transaction);
  ...
  const handleFilterChange = (e) => {
    ...
    setFilters((prev) => ({ ...prev, [name]: value === "Tất cả" ? "" : value }));
  };
  ...
  return (
    <div className="min-h-screen w-full bg-[#f5f6fa] px-4 py-2 ">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-white p-4 rounded-md shadow">
        {/* Loại */}
        <Select label="Loại" name="type" value={filters.type} options={typeOptions} onChange={handleFilterChange}
          render={(o) => (o === "income" ? "Thu" : o === "expense" ? "Chi" : o)} />
        {/* Mục */}
        <Select label="Mục giao dịch" name="category" value={filters.category} options={categoryOptions} onChange={handleFilterChange} />
        {/* Tháng */}
        <Select label="Tháng" name="month" value={filters.month} options={["Tất cả", ...Array.from({ length: 12 }, (_, i) => i + 1)]} onChange={handleFilterChange} />
        {/* Năm */}
        <Select label="Năm" name="year" value={filters.year} options={["Tất cả", ...years]} onChange={handleFilterChange} />
      </div>
...
const Select = ({ label, name, value, options, onChange, render }) => (
  <div>
    ...
    <select name={name} value={value} onChange={onChange} className="w-full border rounded px-3 py-2 text-gray-600">
      {options.map((opt) => (
