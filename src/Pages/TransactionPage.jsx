  ...
  return (
    <div className="min-h-screen w-full bg-[#f5f6fa] px-4 py-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-white p-4 rounded-md shadow">
        <Select label="Loại" name="type" value={type} options={typeOptions} onChange={handleFilterChange} render={(o) => (o === "income" ? "Thu" : o === "expense" ? "Chi" : o)} />
        <Select label="Mục giao dịch" name="category" value={category} options={categoryOptions} onChange={handleFilterChange} />
        <Select label="Tháng" name="month" value={month} options={["Tất cả", ...Array.from({ length: 12 }, (_, i) => String(i + 1))]} onChange={handleFilterChange} />
        <Select label="Năm" name="year" value={year} options={years} onChange={handleFilterChange} />
      </div>
      ...
      <div className="bg-white mt-6 rounded-md shadow flex flex-col divide-y sm:flex-row sm:divide-y-0 sm:divide-x">
        ...
        <Summary title="Số lượng giao dịch" value={total} color="blue" isCount />
      </div>

      <div className="flex justify-end mt-6 mb-2">
        <button onClick={handleAdd} className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold text-sm py-2 px-4 rounded w-fit flex items-center gap-2 cursor-pointer">
        ...
        <table className="w-full text-left text-sm mt-4 md:text-base">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="6" className="text-center py-4">Không có giao dịch</td></tr>
            ) : (
              transactions.map((t, i) => (
                ...
                >
                  <td className="hidden sm:table-cell py-2">{t.type === "income" ? "Thu" : "Chi"}</td>
                  <td>{t.category}</td>
                  ...
                  <td className="py-2 text-right">
                    <span className="inline-flex gap-2 text-gray-600">
                      <FaEdit className="cursor-pointer hover:text-blue-500" onClick={(e) => { e.stopPropagation(); handleEdit(t); }} />
                      <FaTrash className="cursor-pointer hover:text-red-500" onClick={(e) => { e.stopPropagation(); dispatch(deleteTransaction(t._id)); }} />
                    </span>
                  </td>
                </tr>
              ))
...
const Summary = ({ title, value, color, isCount = false }) => (
  <div className="flex-1 p-4 sm:text-center">
    ...
    <p className={`text-${color}-600 font-bold text-xl`}>
      {isCount ? value : `${color === "green" ? "+" : "-"}${value.toLocaleString("vi-VN")}`}
    </p>
  </div>
);
