...
const BudgetPageLoading = () => {
  return (
    <section className="relative w-full px-2 py-4 flex flex-col gap-4 items-center sm:p-4 lg:p-6 xl:w-[90%] xl:mx-auto">
      {/* Title */}
      <SkeletonBox className="self-start w-32 h-8" />
      ...
      <div className="w-full flex flex-col gap-3 lg:grid lg:grid-cols-[65%_35%]">
        ...
        {/* Summary box */}
        <section className="w-full p-3 bg-white rounded flex items-center gap-2 sm:p-4 sm:gap-0 lg:order-1">
          <div className="flex-1 flex flex-col gap-5 sm:gap-4 sm:p-3 md:gap-7">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="flex gap-2 items-center md:gap-5">
                <SkeletonBox className="w-24 h-4 md:w-30 md:p-2" />
                <SkeletonBox className="w-24 h-4 md:w-30 md:p-2" />
              </div>
            ))}
          </div>
          ...
          <div className="flex-1 self-center flex flex-col items-center gap-4">
            {/* Circle placeholder */}
            <SkeletonBox className="w-28 h-28 rounded-full md:w-35 md:h-35" />

            {/* Legend (mobile 3 dưới, tablet trở lên: 5 bên phải) */}
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              {[...Array(2)].map((_, i) => (
                <div key={i} className="flex items-center gap-2">
                  <SkeletonBox className="w-4 h-4 rounded-full bg-slate-300" />
      ...
      {/* Categories */}
      <section className="w-full p-3 bg-white rounded flex flex-col gap-4 sm:p-4 lg:p-6">
        ...
        {[...Array(3)].map((_, idx) => (
          <div key={idx} className="w-full flex flex-col gap-2">
            ...
            <hr className="text-[#A5A5A5] h-[1px] w-full" />
          </div>
        ))}
      </section>
    </section>
